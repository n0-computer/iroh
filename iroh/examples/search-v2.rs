...
//!  cargo run -- example search --features=example 
...
async fn main() -> Result<()> {
    ...
    match args.command {
        ...
        Command::Query { node_id, query } => {
            ...
            let (num_matches, matches) = proto.query_remote(node_id, &query).await?;

            // Print out our query results.
            println!("Found {} matches with query: {}", num_matches, query);
            println!("Matching content:");
            for (i, text) in matches.iter().enumerate() {
                println!("{}. {}", i + 1, text);
            }
        }
    }
...
impl ProtocolHandler for BlobSearch {
    ...
    fn accept(&self, connection: Connection) -> BoxFuture<Result<()>> {
        ...
        Box::pin(async move {
            ...
            // Get both matches and their content
            let (matches, content) = this.query_local(&query).await;
            
            // First send the number of matches
            send.write_all(&matches.to_le_bytes()).await?;
            
            // Then send each matching string, prefixed with its length
            for text in content {
...
impl BlobSearch {
    ...
    pub async fn query_remote(&self, node_id: NodeId, query: &str) -> Result<(u64, Vec<String>)> {
    ...
    ///
    /// Returns the number of matches and a vector of matching strings.
    pub async fn query_local(&self, query: &str) -> (u64, Vec<String>) {
