...
impl MagicSock {
    ...
    fn poll_recv_relay(
        ...
        match self.relay.poll_recv(cx, bufs, &mut inner_metas) {
            Poll::Ready(Ok(msgs)) => {
                for ((buf, meta), out_meta) in bufs
                    .iter()
                    .take(msgs)
                    .zip(inner_metas.iter().take(msgs))
                    .zip(metas.iter_mut().take(msgs))
                {
                    match self.process_relay_read_result(buf, meta) {
                        None => {
