...
impl TransportsSender {
    fn ip_v4_senders(&self) -> impl Iterator<Item = &IpSender> {
        self.ip_v4.iter().chain(self.ip_v4_default.iter())
    }

    fn ip_v6_senders(&self) -> impl Iterator<Item = &IpSender> {
        self.ip_v6.iter().chain(self.ip_v6_default.iter())
    }
    ...
    pub(crate) async fn send(
        ...
        match dst {
            ...
            Addr::Ip(addr) => match addr {
                SocketAddr::V4(_) => {
                    for sender in self.ip_v4_senders().filter(|s| s.is_valid_send_addr(addr)) {
                        any_match = true;
                ...
                SocketAddr::V6(_) => {
                    for sender in self.ip_v6_senders().filter(|s| s.is_valid_send_addr(addr)) {
                        any_match = true;
                        match sender.send(*addr, src, transmit).await {
                            Ok(()) => {
                                trace!("sent");
