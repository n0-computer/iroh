            ...
            probes.spawn(
                async move {
                        ...
                        .run_until_cancelled_owned(async move {
                            ...
                            while let Some(res) = set.join_next().await {
                                match res {
                                    ...
                                    Ok(Some(Ok(Err(ProbeError::Error(err, probe))))) => {
                                        probe_proto = Some(probe.proto());
                                        warn!(?probe, "probe failed: {:#}", err);
                                        continue;
                                    }
                                    Ok(Some(Ok(Err(ProbeError::AbortSet(err, probe))))) => {
                                        debug!(?probe, "probe set aborted: {:#}", err);
                                        set.abort_all();
                                        return ProbeFinished::Regular(Err(err));
                                    }
                            ...
                            ProbeFinished::Regular(Err(anyhow!("All probes in ProbeSet failed")))
                        })
                        .await;
                    ...
                    match res {
                        ...
                        None => {
                            debug!("probe cancelled");
                            ProbeFinished::Regular(Err(anyhow!("probe cancelled")))
                        }
                    }
                }
