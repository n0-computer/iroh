...
use crate::rpc::proto::{net::{self, AddAddrRequest, AddrRequest, IdRequest, NodeWatchRequest, RelayRequest, RemoteInfoRequest, RemoteInfoResponse, RemoteInfosIterRequest, RemoteInfosIterResponse, WatchResponse}, RpcError, RpcResult, RpcService};
...
impl NetServer {
    pub async fn handle_rpc_request<C>(self, msg: net::Request, chan: RpcChannel<RpcService, C>) -> Result<(), RpcServerError<C>>
    ...
    fn remote_infos_iter(
        ...
        futures_lite::stream::iter(infos.into_iter().map(|info| Ok(RemoteInfosIterResponse { info })))
    }
