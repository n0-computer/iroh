...
use iroh_rpc_types::{
    gateway::GatewayClientAddr,
    p2p::P2pClientAddr,
    qrpc::{addr::Addr, gateway::GatewayService, p2p::P2pService, store::StoreService},
    store::StoreClientAddr,
};
...
mod tests {
    ...
    #[test]
    fn test_collect2() {
        let default = Config2::default_qrpc();
        let mut expect: Map<String, Value> = Map::new();
        expect.insert(
            "gateway_addr".to_string(),
            Value::new(None, default.gateway_addr.unwrap().to_string()),
    ...
    #[test]
    fn test_build_config_from_struct2() {
        let expect = Config2::default();
        let got: Config2 = ConfigBuilder::builder()
            .add_source(Config2::default())
            .build()
            .unwrap()
            .try_deserialize()
            .unwrap();
