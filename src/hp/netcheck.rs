...
use std::{time::{Duration, SystemTime}, collections::HashMap, net::IpAddr};

use tokio::sync::Mutex;
use tracing::{debug, info};

use super::derp::{DerpRegion, DerpMap};
...
pub struct Report {
    ...
    /// keyed by DERP Region ID
    pub region_v6_latenc: HashMap<int, Duration>,
...
impl Client {
    ...
    async fn handle_hair_stun_locked(&self, pkt: &[u8], src: IpAddr) -> bool {
        ...
        if let Some(ref rs) = reports.cur_state {
            let decoder = stun_rs::MessageDecoder::default();
            if let Ok((msg, _)) = decoder.decode(pkt) {
                if msg.transaction_id() == rs.hair_tx {
                    // TODO:
                    // rs.gotHairSTUN <- src:
	            return true;
                }
	...
	if let Ok(tx, addrPort) = stun.ParseResponse(pkt) {
	    if stun.ParseBindingRequest(pkt).is_ok() {
		// This was probably our own netcheck hairpin
		// check probe coming in late. Ignore.
		return
	    }
	    info!("netcheck: received unexpected STUN message response from {}: {:?}", src, err);
	    return;
	}

        let reports = &mut *self.reports.lock().await;
        if let Some(ref mut rs) = reports.cur_state {
	    if let Some(on_done) = rs.in_flight.remove(tx) {
                drop(reports);
		on_done(addr_port);
	    }
        }
...
fn sort_regions(dm: &DerpMap, last: &Report) -> Vec<&DerpRegion> {
...
fn make_probe_plan(dm: &DerpMap, ifState *interfaces.State, last: &Report) -> ProbePlan {
