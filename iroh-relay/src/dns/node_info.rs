...
use iroh_base::{NodeAddr, NodeId, SecretKey, SignatureError};
use tracing::warn;
use url::Url;
...
impl NodeInfo {
    ...
    pub fn from_txt_lookup(lookup: super::TxtLookup) -> Result<Self, Error> {
...
impl<T: FromStr + Display + Hash + Ord> TxtAttrs<T> {
    ...
    pub(super) async fn lookup_by_name(resolver: &DnsResolver, name: &str) -> Result<Self, Error> {
    ...
    pub(super) fn from_pkarr_signed_packet(packet: &pkarr::SignedPacket) -> Result<Self, Error> {
        use pkarr::dns::{
            rdata::RData,
            {self},
        };
    ...
    pub(super) fn from_txt_lookup(lookup: super::TxtLookup) -> Result<Self, Error> {
