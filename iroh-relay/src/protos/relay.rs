    ...
    pub(crate) fn from_bytes(mut content: Bytes, cache: &KeyCache) -> Result<Self, Error> {
        ...
        let res = match frame_type {
            FrameType::RelayToClientDatagram | FrameType::RelayToClientDatagramBatch => {
                ensure!(content.len() >= PublicKey::LENGTH, Error::InvalidFrame);

                let remote_endpoint_id = cache.key_from_slice(&content[..PublicKey::LENGTH])?;
                let datagrams = Datagrams::from_bytes(
                    content.slice(PublicKey::LENGTH..),
    ...
    pub(crate) fn from_bytes(mut content: Bytes, cache: &KeyCache) -> Result<Self, Error> {
        ...
        let res = match frame_type {
            FrameType::ClientToRelayDatagram | FrameType::ClientToRelayDatagramBatch => {
                let dst_endpoint_id = cache.key_from_slice(&content[..PublicKey::LENGTH])?;
                let datagrams = Datagrams::from_bytes(
                    content.slice(PublicKey::LENGTH..),
...
mod proptests {
    ...
    fn datagrams() -> impl Strategy<Value = Datagrams> {
        ...
        const MAX_PAYLOAD_SIZE: usize = MAX_PACKET_SIZE - PublicKey::LENGTH - 1 /* ECN bytes */ - 2 /* segment size */;
        (
            ecn(),
