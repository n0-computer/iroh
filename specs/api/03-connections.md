...
These are combined into an **Endpoint Address** (see [Addressing](addressing.md)).
...
3. The TLS handshake authenticates both peers using their Ed25519 keys (see [Identity](identity.md)).
4. The fastest path to complete the handshake is used. If the relay path wins, iroh begins hole punching to establish a direct connection in the background (see [Holepunching](holepunching.md)).
