use crate::{config::Config, mem_p2p, mem_store};
...
impl<'a> Iroh<'a> {
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        self.config.metrics =
            metrics::metrics_config_with_compile_time_info(self.config.metrics.clone());
        println!("{:#?}", self.config);

        let metrics_config = self.config.metrics.clone();

        let gateway_rpc_addr = self
            .config
            .gateway
            .server_rpc_addr()?
        ...
        let handler =
            iroh_gateway::core::Core::new_with_state(gateway_rpc_addr, Arc::clone(&shared_state))
                .await?;
