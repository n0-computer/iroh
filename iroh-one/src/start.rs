...
pub async fn run_start_command(args: &Args) -> Result<()> {
    ...
    let (store_rpc, p2p_rpc) = {
        ...
        let store_rpc = crate::mem_store::start(store_recv, config.clone().store).await?;

        let (p2p_recv, p2p_sender) = Addr::new_mem();
        config.rpc_client.p2p_addr = Some(p2p_sender);
        let p2p_rpc = crate::mem_p2p::start(p2p_recv, config.clone().p2p).await?;
        (store_rpc, p2p_rpc)
    };
    ...
    let content_loader =
        crate::content_loader::RacingLoader::new(RpcClient::new(config.rpc_client.clone()).await?);
