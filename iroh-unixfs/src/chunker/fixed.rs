                ...
                match source.read_buf(&mut buffer).await {
                    Ok(len) => {
                        ...
                        if current_len == chunk_size {
                            // read a full chunk
                            current_len = 0;
                            let value = source.enhance(buffer.clone().freeze());
                            yield Ok(value);
                            ...
                            let value = source.enhance(out.freeze());
                            yield Ok(value);
                        } else {
                            // finished reading
                            debug_assert!(len == 0);
...
mod tests {
    ...
    async fn test_fixed_chunker() {
        // exact match
        {
            ...
            let bytes = ReaderWithProvenance::from(std::io::Cursor::new(content));

            let chunks: Vec<_> = Fixed::new(256).chunks(bytes).try_collect().await.unwrap();
            assert_eq!(chunks.len(), 4);
            assert_eq!(&chunks[0].data, &[1u8; 256][..]);
            assert_eq!(&chunks[1].data, &[2u8; 256][..]);
            assert_eq!(&chunks[2].data, &[3u8; 256][..]);
            assert_eq!(&chunks[3].data, &[4u8; 256][..]);
        }
