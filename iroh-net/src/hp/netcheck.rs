...
use iroh_metrics::{core::Metric, netcheck::Metrics as NetcheckMetrics};
...
impl Client {
    ...
    pub fn receive_stun_packet(&self, payload: Bytes, src: SocketAddr) {
        ...
        {
            NetcheckMetrics::with_metric(|m| m.stun_packets_dropped.inc());
    ...
    match probe {
        Probe::Ipv4 { .. } => {
            if let Some(ref pc4) = pc4 {
                ...
                NetcheckMetrics::with_metric(|m| m.stun_packets_sent_ipv4.inc());
        ...
        Probe::Ipv6 { .. } => {
            if let Some(ref pc6) = pc6 {
                ...
                NetcheckMetrics::with_metric(|m| m.stun_packets_sent_ipv6.inc());
...
impl Actor {
        ...
        tokio::spawn(async move {
            ...
            .await
            {
                ...
                Err(err) => {
                    ...
                    NetcheckMetrics::with_metric(|m| m.reports_error.inc());
    ...
    async fn create_report_state(
        ...
        if do_full {
            ...
            NetcheckMetrics::with_metric(|m| m.reports_full.inc());
        }
        NetcheckMetrics::with_metric(|m| m.reports.inc());
    ...
    fn handle_stun_packet(&mut self, pkt: &[u8], src: SocketAddr) {
        ...
        match &src {
            SocketAddr::V4(_) => {
                NetcheckMetrics::with_metric(|m| m.stun_packets_recv_ipv4.inc());
            }
            SocketAddr::V6(_) => {
                NetcheckMetrics::with_metric(|m| m.stun_packets_recv_ipv6.inc());
            }
        }
